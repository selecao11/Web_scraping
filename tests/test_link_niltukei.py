...
def test_niltukei_succes1():
    ...
    file_path = './'
    file_name = kb_title + '_link_株価.csv'
    ...
    join_shinyou_zan_df = pd.read_csv(csv_path + 'link_tests/三菱自動車_link_信用残.csv')
    join_gyakuhibu_taisyaku_df = pd.read_csv(csv_path + 'link_tests/三菱自動車_link_逆日歩_貸借桟.csv')
