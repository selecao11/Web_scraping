...
class StockPriceAccumulation:
    ...
    def colum_drop(self):
        self.accumulation_df = self.accumulation_df.drop(
            Niltukei_const.UNNAMED_0_KOUMOKU, axis=1)
        '''self.accumulation_df = self.accumulation_df.drop\
            ('index', axis=1) '''
    def stock_price_accumulation_sort(self):
        self.accumulation_df = self.accumulation_df.sort_values(
            '日付', ascending=False)
    ...
    def stock_price_accumulation(self):
        self.accumulation_df = pd.concat([self.stock_price_accumulation_df,
                                          self.difference_df], axis=0)
